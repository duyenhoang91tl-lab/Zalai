...
(function () {
  ...
  let _careStatusTree = JSON.parse(JSON.stringify(CARE_STATUS_TREE_DEFAULT));

  const CUSTOMER_STATUS_TREE = [
  ...
  function buildPanel() {
    ...
    buildStatusSelect_(statusSel, _careStatusTree, '');
    ...
    const csSel = addEl(upd, 'input', {type:'hidden', id:'zai-cs-sel'});
    const nzFormSel = addEl(upd, 'input', {type:'hidden', id:'zai-nz-form-sel'});

    addEl(upd, 'label', {textContent:'Trạng thái KH'});
    const khStatusSel = addEl(upd, 'select', {id:'zai-kh-status-sel'});
    buildStatusSelect_(khStatusSel, CUSTOMER_STATUS_TREE, '');
    ...
    addEl(upd, 'label', {textContent:'🎂 Sinh nhật khách'});
    addEl(upd, 'input', {id:'zai-birthday', type:'date'});
  ...
  function buildStatusSelect_(sel, tree, selected) {
    ...
    tree.forEach(node => {
      if (node.children) {
        ...
        node.children.forEach(child => {
          ...
          o.value = child.value;
          o.textContent = node.label + ' - ' + (child.label || child.value);
          if (selected === child.value) o.selected = true;
          grp.appendChild(o);
        });
        sel.appendChild(grp);
      } else if (node.value) {
        const o = document.createElement('option');
        o.value = node.value;
        o.textContent = node.label || node.value;
        if (selected === node.value) o.selected = true;
        sel.appendChild(o);
      }
    });
  }
  ...
  async function saveConfig() {
    ...
    _lookupCache = {};
    loadCSNames_(); loadCareStatusTree_();
  ...
  function clearForm_() {
    ['zai-status-sel','zai-zalo-sel','zai-hen-date','zai-hen-note','zai-birthday','zai-note-ta']
  ...
  function renderCard_(area, updSec, phone, raw, care, orders) {
    ...
    updSec.style.display = 'block';
    const statusSel = document.getElementById('zai-status-sel');
    buildStatusSelect_(statusSel, _careStatusTree, care&&care.status||'');
    const khSel = document.getElementById('zai-kh-status-sel');
    buildStatusSelect_(khSel, CUSTOMER_STATUS_TREE, care&&care.khStatus||'');
    ...
    const nzF = document.getElementById('zai-nz-form-sel');
    if (nzF) nzF.value = _currentZaloNick || (care&&care.nickZalos&&care.nickZalos[0]||'');
  }
  ...
  async function doSaveStatus() {
    ...
    const birthday = document.getElementById('zai-birthday').value;
  ...
  function renderReminderPanel_(reminders) {
    ...
    reminders.forEach(rem => {
      ...
      openBtn.addEventListener('click', () => {
        navigator.clipboard.writeText(rem.phone).catch(()=>{});
        const inp = document.getElementById('zai-phone-input');
        if (inp) { inp.value = rem.phone; doLookup(); }
      });
      // Done
...
async function startReminderPoll_() {
  ...
  async function loadNickZaloList_() {
    ...
    try {
      ...
      const nzF = document.getElementById('zai-nz-form-sel');
      if (nzF && !nzF.value) nzF.value = _currentZaloNick || '';
    } catch(e) {}
  }
  ...
  async function loadCareStatusTree_() {
    ...
    try {
      const sep = GAS_URL.includes('?') ? '&' : '?';
      // Try getSetting first (lightweight), fall back to action=customers
      let tree = null;
      try {
        const r1 = await fetch(GAS_URL + sep + 'action=getSetting&key=careStatus', {redirect:'follow'});
        const d1 = await r1.json();
        if (d1.value) {
          const parsed = JSON.parse(d1.value);
          if (Array.isArray(parsed) && parsed.length) tree = parsed;
        }
      } catch(e1) {}
      if (!tree) {
        const r2 = await fetch(GAS_URL + sep + 'action=customers', {redirect:'follow'});
        const d2 = await r2.json();
        if (d2.careStatus && Array.isArray(d2.careStatus) && d2.careStatus.length) {
          tree = d2.careStatus;
        }
      }
      if (tree) {
        _careStatusTree = tree;
        const sel = document.getElementById('zai-status-sel');
        if (sel) buildStatusSelect_(sel, _careStatusTree, sel.value);
      }
    } catch(e) {}
  }
  ...
  async function loadCSNames_() {
    ...
    try {
      ...
      if (d.users && d.users.length) {
        ...
        if (sel && !sel.value) sel.value = _currentCS || '';
      }
    } catch(e) {}
  }
